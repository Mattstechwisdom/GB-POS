...
assert.doesNotMatch(main, /createUpdateRelaunchGuard/, 'The updater must use the previously working native electron-updater handoff without a competing retry installer.');
assert.doesNotMatch(main, /GBPOS_UPDATE_GUARD/, 'The removed PowerShell retry guard must not remain armed in the app.');
