...
assert.match(main, /downloadedUpdateInstallerPath\s*=\s*resolveDownloadedInstallerPath\(downloadedFiles\)/, 'The updater must capture the real installer path returned by downloadUpdate.');
assert.doesNotMatch(main, /buildWindowsUpdateHandoff\(downloadedUpdateInstallerPath, process\.pid\)/, 'Windows updates must not bypass electron-updater installation-directory and elevation handling.');
assert.doesNotMatch(main, /setTimeout\(\(\) => app\.exit\(0\)/, 'The POS must not quit independently before electron-updater owns the installer handoff.');
...
assert.match(main, /autoUpdater\.quitAndInstall\(true,\s*true\)/, 'Every desktop update must use electron-updater\'s native silent install and relaunch handoff.');
