...
assert.match(main, /buildWindowsUpdateHandoff\(downloadedUpdateInstallerPath, process\.pid\)/, 'Windows updates must launch only after the current POS process exits.');
assert.match(main, /app\.exit\(0\)/, 'The prepared POS process must terminate after handing control to the waiting launcher.');
...
assert.match(main, /autoUpdater\.quitAndInstall\(true,\s*true\)/, 'Non-Windows platforms must retain the native updater fallback.');
