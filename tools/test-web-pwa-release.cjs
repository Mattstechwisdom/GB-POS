...
assert.match(read('src/workorders/releasePrint.ts'),/durantTicket=/); const url='Web Interface: https://mattstechwisdom.github.io/GB-POS'; assert.ok(read('docs/WEB-INTERFACE.txt').includes(url)); assert.ok(read('tools/release-notes/v0.6.27.md').includes(url)); assert.ok(read('.github/workflows/release.yml').includes(url));
