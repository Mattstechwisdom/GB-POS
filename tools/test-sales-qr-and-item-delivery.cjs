...
assert.match(receipt,/qrRecordType/);assert.match(receipt,/qrGetStatusUrl\?\.\(qrRecordType, recordId\)/);assert.match(receipt,/Update QR/);
assert.match(receipt,/workOrderId/, 'Work-order receipts must resolve their saved record identity before QR generation.');
...
assert.match(receipt,/disabled=\{shouldRenderStatusQr && \(!qrReady \|\| !qrDataUrl\)\}/,'Manual printing must wait for a verified record-specific QR.');
