...
function emailCopy(details: ReturnType<typeof customerDetails>, statusKey: string, statusLabel: string, estimatedDate: string, estimatedTime: string, notes: string, responseUrl = '') {
  ...
  const actionLink = (action:string,label:string,color:string) => `<a href="${escapeHtml(responseUrl)}&action=${action}" style="display:inline-block;margin:4px;padding:12px 16px;background:${color};color:white;text-decoration:none;font-weight:bold">${label}</a>`;
  const pickupAction = ['repair_complete','not_possible','repair_declined','pickup_reminder','schedule_pickup'].includes(statusKey);
  const actions = responseUrl ? `<div style="margin:22px 0;display:flex;gap:9px;flex-wrap:wrap">${statusKey==='repair_approval'?actionLink('approve','Approve Repair','#15803d')+actionLink('decline','Decline Repair','#be123c'):''}${pickupAction?actionLink('confirm_pickup','Confirm Pickup','#15803d')+actionLink('request_pickup_change','Request Another Time','#b45309'):''}${actionLink('question','Ask a Question','#7e22ce')}${actionLink('add_information','Add Information','#0369a1')}</div>` : '';
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if(type==='repair' && deliveryMode !== 'internal'){
      ...
      await admin.from('client_response_tokens').insert({shop_id:profile.shop_id,work_order_id:record.id,legacy_record_id:legacyRecordId,token_hash:digest,allowed_actions:allowedActions,expires_at:new Date(Date.now()+30*86400000).toISOString()});
