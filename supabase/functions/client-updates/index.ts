...
function emailCopy(details: ReturnType<typeof customerDetails>, statusKey: string, statusLabel: string, estimatedDate: string, estimatedTime: string, notes: string, responseUrl = '') {
  ...
  const actions = responseUrl ? `<div style="margin:22px 0"><a href="${escapeHtml(responseUrl)}" style="display:inline-block;padding:13px 20px;border-radius:6px;background:#7e22ce;color:white;text-decoration:none;font-weight:bold">Click to Respond</a></div>` : '';
