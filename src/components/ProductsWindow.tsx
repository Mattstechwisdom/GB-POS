...
type ProductsWindowProps = {
  onClose?: () => void;
  pickerMode?: boolean;
  onPick?: (product: Record<string, any> | Record<string, any>[]) => void;
};
...
const ProductsWindow: React.FC<ProductsWindowProps> = ({ onClose, pickerMode = false, onPick }) => {
  ...
  const [pickerSelections, setPickerSelections] = useState<Set<number>>(() => new Set());
  ...
  const emitPickedProduct = () => {
    ...
    const ids = pickerSelections.size > 0 ? Array.from(pickerSelections) : [selectedId];
    const payloads = ids.map((id) => {
      const stored = list.find(product => Number(product.id) === id);
      if (!stored) return null;
      const product = {
        ...(id === selectedId ? { ...stored, ...editing } : stored),
        id,
        quantity: id === selectedId ? pickerQuantity : (pickerQuantities[id] || 1),
        inStock: id === selectedId ? pickerInStock : (stored.trackStock ? Number(stored.stockCount) > 0 : true),
      };
      return buildSaleProductPickerPayload(product as Record<string, any>);
  ...
  const focusPickerProduct = (product: Product) => {
  ...
  const togglePickerProduct = (product: Product) => {
    const id = Number(product.id);
    if (!Number.isFinite(id)) return;
    setSelectedId(id);
    setPickerSelections(current => {
  ...
  useEffect(() => {
    ...
    setPickerQuantity(pickerQuantities[selectedId] || 1);
  ...
  }, [selectedId, list, pickerQuantities]);
    ...
    return (
      <div className={`gb-sale-catalog-picker ${selectedId ? 'has-selection' : ''} flex h-full min-h-0 w-full flex-col bg-zinc-900 p-3 text-gray-100`}>
        ...
        <div className="gb-sale-catalog-layout grid min-h-0 flex-1 grid-cols-[minmax(360px,0.9fr)_minmax(420px,1.1fr)] gap-3 pt-3">
          ...
          <section className="gb-sale-catalog-editor min-h-0 overflow-hidden rounded-xl border border-zinc-700 bg-zinc-950 p-4">
            {!selectedId ? <div className="grid h-full min-h-40 place-items-center text-sm text-zinc-400">Select a product from the organized inventory list.</div> : (
              <div className="gb-sale-catalog-editor-content space-y-3">
                <div className="flex items-center justify-between gap-3 border-b border-zinc-800 pb-3"><button type="button" className="gb-mobile-catalog-back rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs text-zinc-200" onClick={() => setSelectedId(undefined)}>Back to products</button><div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#39FF14]">Inventory selection</div><h3 className="font-semibold">Add to sale</h3></div><span className="rounded border border-[#BC13FE]/60 bg-[#BC13FE]/15 px-2 py-0.5 text-xs text-[#e9b7ff]">Sale only</span></div>
                <div className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-3"><strong className="block text-lg text-white">{editing.itemDescription || 'Unnamed product'}</strong><div className="mt-2 flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-zinc-700 px-2 py-1">{editing.category || 'Other'}</span><span className="rounded-full border border-zinc-700 px-2 py-1">{editing.condition || 'New'}</span>{editing.distributor ? <span className="rounded-full border border-zinc-700 px-2 py-1">{editing.distributor}</span> : null}</div><p className="mt-2 text-[11px] text-zinc-500">Catalog details are managed in Admin → Inventory.</p></div>
                ...
                <label className="flex items-center justify-between gap-2 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-3 text-sm"><span><strong className="block">Available for this sale</strong><small className="text-zinc-500">Turn off to send it to the ordering workflow.</small></span><input type="checkbox" checked={pickerInStock} onChange={e => setPickerInStock(e.target.checked)} className="h-5 w-5 accent-[#39FF14]" /></label>
