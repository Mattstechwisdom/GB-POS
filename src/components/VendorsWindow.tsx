...
export default function VendorsWindow({ embedded = false }: { embedded?: boolean }) {
  ...
  const remove = async () => {
    ...
    const links = groupVendorLinks(editing, products, repairs);
    if (links.parts.length || links.repairs.length) return alert(`Reassign or merge this distributor first. It is linked to ${links.parts.length} part(s) and ${links.repairs.length} repair(s).`);
