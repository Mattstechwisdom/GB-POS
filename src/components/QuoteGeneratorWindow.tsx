            ...
            <div key={`extra-${i}`} className="col-span-16">
              <div className="grid grid-cols-16 gap-2 items-start">
                ...
                <div className="col-span-1 flex items-end justify-end">
                  <button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" onClick={() => setSales((s)=>({ ...s, items: s.items.map((x,ii)=>{ if(ii!==idx) return x; const list = Array.isArray((x.dynamic as any)?.extraParts)?[...(x.dynamic as any).extraParts]:[]; list.splice(i,1); return { ...x, dynamic: { ...(x.dynamic||{}), extraParts: list } }; }) }))}>✕</button>
                </div>
              </div>
            </div>
          ))}
                ...
                <div className="flex flex-col gap-2">
                  {specs.map((s, si) => (
                    <div key={`other-spec-${si}`} className="flex items-center gap-2">
                      ...
                      <button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" onClick={() => removeOtherSpec(idx, si)}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                {specs.map((s, si) => (
                  <div key={`drone-spec-${si}`} className="flex items-center gap-2">
                    ...
                    <button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" onClick={() => removeDroneSpec(idx, si)}>✕</button>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div key={idx} className="border border-zinc-700 rounded p-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex justify-end gap-1">
                        <button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" title={it.expanded ? 'Hide details' : 'Show details'} onClick={() => toggleSaleItemExpanded(idx)}>{it.expanded ? '▴' : '▾'}</button>
                        <button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" onClick={() => removeSaleItem(idx)}>✕</button>
                      </div>
                    </div>
                    {it.expanded && (
                            ...
                            {it.images && it.images.length > 0 ? (
                              it.images.map((src, i) => (
                                <div key={i} className="relative w-20 h-20 flex-none border border-zinc-700 rounded overflow-hidden">
                                  ...
                                  <button className="absolute top-0 right-0 m-0.5 bg-black/70 text-white text-[10px] leading-none px-1 rounded" onClick={() => removeImageFromItem(idx, i)} title="Remove">✕</button>
                                </div>
                              ))
                            ) : (
            ...
            <div className="space-y-2">
              {repairs.lines.map((ln, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                  ...
                  <div className="col-span-1 flex justify-end"><button className="px-2 py-1 text-xs bg-zinc-700 border border-zinc-600 rounded" onClick={() => removeRepairLine(idx)}>✕</button></div>
                </div>
              ))}
            </div>
