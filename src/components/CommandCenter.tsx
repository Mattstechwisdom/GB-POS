...
export default function CommandCenter(props: Props) {
  const [data, setData] = useState({ customers: [] as any[], technicians: [] as any[], workOrders: [] as any[], sales: [] as any[], calendarEvents: [] as any[], purchaseOrders: [] as any[], attentionSettings: undefined as any });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({ customers: customers || [], technicians: technicians || [], workOrders: reconciledWorkOrders, sales: sales || [], calendarEvents: calendarEvents || [], purchaseOrders: purchaseOrders || [], attentionSettings });
  ...
  useEffect(() => {
    ...
    const onWorkOrderChanged = (record?: any) => {
      if (record?.id != null) setData(current => ({ ...current, workOrders: upsertCommandCenterWorkOrder(current.workOrders, record) }));
      void load();
    };
    const offs = [api?.onWorkOrdersChanged?.(onWorkOrderChanged), api?.onSalesChanged?.(load), api?.onCustomersChanged?.(load), api?.onTechniciansChanged?.(load), api?.onCalendarEventsChanged?.(load), api?.onPurchaseOrdersChanged?.(load)];
  ...
  useEffect(()=>subscribeWorkOrderUpdates(record=>setData(current=>({...current,workOrders:upsertCommandCenterWorkOrder(current.workOrders,record)}))),[]);
  ...
  const markProductsDelivered = async (record: CommandCenterRecord, selectedIndexes?: number[]) => {
  ...
  const todayRows: Record<string, any[]> = { Tasks: model.today.tasks, Events: model.today.events, Consultations: model.today.consultations, Deliveries: model.today.deliveries };
  const todayLabel = (row: any) => row?.title || row?.name || row?.partName || row?.deviceLabel || row?.source?.title || row?.source?.productDescription || 'Scheduled item';
  const todayDetail = (row: any) => row?.description || row?.notes || row?.customerName || row?.source?.customerName || row?.distributor || row?.category || row?.type || 'No additional details';
  ...
  return <div className="command-center">
    <div className="command-center-heading"><div><h1>Command Center</h1><span>{new Date().toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}</span></div><div><button onClick={() => setPanel({title:'Repair Statistics',kind:'statistics'})}>Repair Statistics</button><button onClick={() => void load()}>Refresh</button></div></div>
