...
export async function reconcileLegacyWorkOrders(api: any, options: { now?: Date; settings?: any; workOrders?: any[] } = {}) {
  ...
  const result = { scanned: workOrders.length, diagnosticOnly: 0, universal: 0, updated: 0, skipped: 0, errors: [] as any[] };
  ...
  for (const workOrder of workOrders) {
    ...
    try { await api.dbUpdate('workOrders', workOrder.id, buildLegacyClosePatch(classification, now, settings)); result.updated += 1; }
