...
const SaleWindow: React.FC = () => {
  ...
  function validate(): boolean {
    const errs: string[] = [];
    // For existing records, allow saving meta changes (e.g., technician) without item validation
    if ((sale as any).id) {
      setErrors([]);
      return true;
    }
    // New sale must have an assigned technician
    if (!sale.assignedTo || !String(sale.assignedTo).trim()) {
      errs.push('Technician required');
    }
    const rows = sale.items || [];
    if (rows.length === 0) {
    ...
    // If in-stock, we ignore date fields; if not in-stock, dates are optional but kept if present
    setErrors(errs);
  ...
  const canSave = useMemo(() => {
    ...
    const hasTech = !!(sale.assignedTo && String(sale.assignedTo).trim());
    const rows = sale.items || [];
     if (rows.length === 0) {
       // Legacy single item fallback
       const desc = (sale as any).itemDescription;
       const qty = Number((sale as any).quantity || 0);
        const price = Number((sale as any).price || 0);
        return !!(hasTech && desc && desc.toString().trim() && qty > 0 && price >= 0);
     }
    for (const r of rows) {
    ...
    return hasTech;
  }, [sale.items, (sale as any).itemDescription, (sale as any).quantity, (sale as any).price, (sale as any).id, sale.assignedTo]);
  ...
  <div className="flex flex-col gap-2 col-span-1 pb-16 min-h-0 overflow-auto">
      ...
      <div className="fixed bottom-4 left-4 right-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <button className="px-3 py-1.5 bg-zinc-800 rounded" onClick={() => window.close()}>Cancel</button>
          <button
